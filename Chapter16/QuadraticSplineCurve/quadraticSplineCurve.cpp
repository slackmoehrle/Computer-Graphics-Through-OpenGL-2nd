...
// 
// This program draws a quadratic B-spline approximation of 9 control points.
// The B-spline curve is drawn red, the control points yellow (except the selected
// control point which is red), and the joints of the curve blue. 
// At the bottom  there are 12 white horizontal bars corresponding, from bottom to 
// top, to the first through twelfth knots, each knot drawn as a green point.
// Also drawn in light gray is the control polygon.
// 
// The knot values can be changed and the control points moved.
//
// Interaction: 
...
void increaseKnot(int i)
{
   ...
   if ( (i == 11 )  && ( knots[i] < 80.0) )    knots[i] += 1.0;
}
...
void drawScene(void)
{
   ...
   gluNurbsCurve(nurbsObject, 12, knots, 3, controlPoints[0], 3, GL_MAP1_VERTEX_3);
   ...
   glBegin(GL_POINTS);
      for (i = 2; i < 10; i++) 
	  {
		 ...
		 for (j = 0; j < 9; j++)
		 {
	        knotImageX += Bspline(j, 3, knots[i]) * controlPoints[j][0];
		    knotImageY += Bspline(j, 3, knots[i]) * controlPoints[j][1];
		 }
         glVertex3f(knotImageX, knotImageY, 0.0);
	  }
   glEnd();

   // Highlight the image of the selected knot point (if it is within the parameter space).
   glColor3f(1.0, 0.0, 0.0);
   glBegin(GL_POINTS);
   knotImageX = knotImageY = 0;
   if ( (2 <= selectedKnot) && (selectedKnot <= 9) )
   {
      for (j = 0; j < 9; j++)
	  {
         knotImageX += Bspline(j, 3, knots[selectedKnot]) * controlPoints[j][0];
         knotImageY += Bspline(j, 3, knots[selectedKnot]) * controlPoints[j][1];
	  }
   glVertex3f(knotImageX, knotImageY, 0.0);
   }   
   glEnd();
...
void keyInput(unsigned char key, int x, int y)
{
   switch (key) 
   {
      ...
      case ' ':
		 ...
		 {
		    if (selectedKnot < 11) selectedKnot++;
		    else selectedKnot = 0;
		 }
         glutPostRedisplay();
         break;
...
int main(int argc, char **argv) 
{
   ...
   glutCreateWindow("quadraticSplineCurve.cpp");
