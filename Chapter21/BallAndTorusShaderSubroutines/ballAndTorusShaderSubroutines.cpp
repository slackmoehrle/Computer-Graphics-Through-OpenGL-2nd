...
void setup(void) 
{
   ...
   // Obtain shader subroutine indices.
   hemSubroutineIndex = glGetSubroutineIndex(programId, GL_VERTEX_SHADER, "hemisphere");
   torSubroutineIndex = glGetSubroutineIndex(programId, GL_VERTEX_SHADER, "torus");
...
void drawScene(void)
{
   ...
   // Draw torus.
   glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &torSubroutineIndex); // Invode torus subroutine.
   glBindVertexArray(vao[TORUS]);
   ...
   glUniformSubroutinesuiv(GL_VERTEX_SHADER, 1, &hemSubroutineIndex); // Invoke hemisphere subroutine.
   glBindVertexArray(vao[HEMISPHERE]);
...
int main(int argc, char **argv) 
{
   ...
   glutCreateWindow("ballAndTorusShaderSubroutines.cpp");
